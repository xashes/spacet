...
def download_index_table():
    ...
    index_df, msg = ds.query(
        ...
        fields='list_date,name,symbol',
        filter='inst_type=100&status=1&symbol=',
        data_format='pandas')
    return index_df

def update_index_table(engine=engine):
    index_df = download_index_table()
    print('Writing to the database.')
    index_df.to_sql(INDEX_TABLE, engine, if_exists='replace', index_label='id')
...
def update_stock_table(engine=engine):
    stock_df = download_stock_table()
    print('Writing to the database.')
    stock_df.to_sql(STOCK_TABLE, engine, if_exists='replace', index_label='id')
